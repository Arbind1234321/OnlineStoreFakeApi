...
public class CartTests extends BaseClass{
	    ...
	    @Test
	    public void testGetUserCartsByDateRange() {
	        ...
	        // Send GET request with path and query parameters
	        given()
	            .pathParam("userId", userId)
	            .pathParam("startdate", startDate)
	            .pathParam("enddate", endDate)
	        .when()
	            .get(Routes.GET_USER_CARTS_BY_DATE_RANGE)
	        .then()
	            .statusCode(200)
	            .contentType(ContentType.JSON);
	            // Basic validations
	         
	          //  .body("userId", equalTo(userId)) // first item has the correct userId
	           // .body("id", notNullValue()) // ensure each cart has an ID
	           // .body("products", notNullValue()); // ensure products list exists
	    }
